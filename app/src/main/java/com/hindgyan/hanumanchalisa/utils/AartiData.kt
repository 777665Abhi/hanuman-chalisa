package com.hindgyan.hanumanchalisa.utils

class AartiData {
    companion object {
        /**Hindi Lyric and meaning*/

        fun hindiList(): ArrayList<String> {
            val hindiArray = ArrayList<String>()
            hindiArray.add(
               "आरती कीजै हनुमान लला की।दुष्ट दलन रघुनाथ कला की॥\n" +
                       "जाके बल से गिरिवर कांपे।रोग दोष जाके निकट न झांके॥"
            )
            hindiArray.add(
                "अंजनि पुत्र महा बलदाई।सन्तन के प्रभु सदा सहाई॥\n" +
                        "दे बीरा रघुनाथ पठाए।लंका जारि सिया सुधि लाए॥"
            )
            hindiArray.add(
                "लंका सो कोट समुद्र-सी खाई।जात पवनसुत बार न लाई॥\n" +
                        "लंका जारि असुर संहारे।सियारामजी के काज सवारे॥"
            )
            hindiArray.add(
                "लक्ष्मण मूर्छित पड़े सकारे।आनि संजीवन प्राण उबारे॥\n" +
                        "पैठि पाताल तोरि जम-कारे।अहिरावण की भुजा उखारे॥"
            )
            hindiArray.add(
               "बाएं भुजा असुरदल मारे।दाहिने भुजा संतजन तारे॥\n" +
                       "सुर नर मुनि आरती उतारें।जय जय जय हनुमान उचारें॥"
            )
            hindiArray.add(
              "कंचन थार कपूर लौ छाई।आरती करत अंजना माई॥\n" +
                      "जो हनुमानजी की आरती गावे।बसि बैकुण्ठ परम पद पावे॥"
            )

            return hindiArray
        }

        fun hindiMeaningList(): ArrayList<String> {
            val hindiMeaningArray = ArrayList<String>()
            hindiMeaningArray.add(
                "सभी दुष्टों का नाश करने वाले और पृथ्वी पर श्री राम के खेल में एक महान नायक, भगवान हनुमान की आरती गाओ।\n"+
                        "उसकी विशाल शक्ति पहाड़ों को भी हिला देती है। उनके भक्तों के निकट न तो रोग और न ही अशुद्धियाँ आ सकती हैं।"
            )

            hindiMeaningArray.add(
                "माता अंजनी का पुत्र अत्यंत शक्तिशाली है। वह हमेशा अच्छे को सहायता और सुरक्षा प्रदान करता है।\n"+
                        "भगवान राम ने बहादुर हनुमान को सीता को खोजने का काम सौंपा, जिसके लिए उन्होंने लंका में छलांग लगा दी और राजधानी को जला दिया।"

            )

            hindiMeaningArray.add(
                "लंका का अभेद्य किला और आसपास के समुद्र की गहरी खाई उसकी प्रगति में बाधा नहीं डाल सकी। वह सीता का समाचार लाने में सफल रहा।\n"+
                        "उन्होंने लंका को जलाया, राक्षसों का वध किया और भगवान राम और माता सीता का कार्य पूरा किया।"
            )

            hindiMeaningArray.add(
                "जब लक्ष्मण बेहोश पड़े थे, तो हनुमान जीवन रक्षक संजीवनी बूटी के लिए तेजी से गए। सुबह होते ही लाकर उन्होंने लक्ष्मण की जान बचाई।\n"+
                        "हनुमान तेजी से मृत्यु के क्षेत्र में गए और अहिरावण की भुजाओं को फाड़ दिया।\n"
            )

            hindiMeaningArray.add(
                "हनुमानजी की शक्तिशाली भुजाएं सदा सद्गुणों की रक्षा और राक्षसों का संहार करने में लगी रहती हैं।\n"+
                "सभी देवता, मनुष्य, ऋषि और द्रष्टा उनकी आरती करते हैं और बड़े उत्साह के साथ 'हनुमान की जय' का जाप करते हैं।"
            )

            hindiMeaningArray.add(
                "हनुमानजी की शक्तिशाली भुजाएं सदा सद्गुणों की रक्षा और राक्षसों का संहार करने में लगी रहती हैं।\n"+
                "जो कोई भी भगवान हनुमान की आरती गाता है वह सर्वोच्च अवस्था को प्राप्त करता है और वैकुंठ, सर्वोच्च स्वर्ग में निवास करता है।\n"+
                "स्वाई तुलसीदास जी हनुमान की महिमा गाते हैं, जिन्होंने भगवान राम के आशीर्वाद से लंका को नष्ट कर दिया था"
            )
            hindiMeaningArray.add(
                "भगवान हनुमान की आरती गाओ, जो सभी दुष्टों का संहारक और पृथ्वी पर श्री राम के खेल में एक महान नायक हैं।\n"
            )
            return hindiMeaningArray
        }

        /**English Lyric and meaning*/

        fun englishList(): ArrayList<String> {
            val englishList = ArrayList<String>()
            englishList.add(
              "Aarti Kije Hanuman Lala Ki।Dusht Dalan Ragunath Kala Ki॥\n" +
                      "Jake Bal Se Girivar Kaanpe।Rog Dosh Ja Ke Nikat Na Jhaanke॥"
            )

            englishList.add(
             "Anjani Putra Maha Baldaaee।Santan Ke Prabhu Sada Sahai॥\n" +
                     "De Beera Raghunath Pathaaye।Lanka Jaari Siya Sudhi Laaye॥"
            )

            englishList.add(
           "Lanka So Kot Samundra-Si Khai।Jaat Pavan Sut Baar Na Lai॥\n" +
                   "Lanka Jaari Asur Sanhare।Siyaramji Ke Kaaj Sanvare॥"
            )
            englishList.add(
             "Lakshman Moorchhit Pade Sakaare।Aani Sajeevan Pran Ubaare॥\n" +
                     "Paithi Pataal Tori Jam-kaare।Ahiravan Ke Bhuja Ukhaare॥"
            )
            englishList.add(
             "Baayen Bhuja Asur Dal Mare।Daahine Bhuja Santjan Tare॥\n" +
                     "Sur Nar Muni Aarti Utare।Jai Jai Jai Hanuman Uchaare॥"
            )
            englishList.add(
             "Kanchan Thaar Kapoor Lau Chhaai।Aarti Karat Anjana Maai॥\n" +
                     "Jo Hanumanji Ki Aarti Gaave।Basi Baikunth Param Pad Pave॥"
            )
            return englishList
        }

        fun englishMeaningList(): ArrayList<String> {
            val englishMeanList = ArrayList<String>()
            englishMeanList.add(" Let us worship Hanuman,  who slays the wicked and is a Rama crescent. \n " +
                    "The great mountains trembled through his power and devils and fantasies never approached us.")

            englishMeanList.add("After the burning down of Sri Lanka, Rama made him good by his path, and he brought good news from Sita.\n" +
                    "The wind god’s son sprang through the sea and in a twinkling of an eye he reached Lanka.")

            englishMeanList.add("With his left hand, Asuras was killed and with his right hand protected the devas \n"+
           "He burned Lanka, murdered the asuras and completed King Rama’s task.")

            englishMeanList.add(" The son of Anjana gives great power and protects the gods and saints forever.\n"+
         "When Lakshmana had fainted and was lying like dead, He brought the Sanjivini and gave him back the life")

            englishMeanList.add("You descended into the below world, broke Yama’s gates and broke Mahi Ravana’s arms.\n"+
            "When devas, sages and men pray you, They chant, victory, victory to king of monkeys.")

            englishMeanList.add("Your mother Anjana prays, Waving a light of the camphor on a golden plate \n"+
            "Those who sing Hanuman this prayer would live and attain salvation in the country of Vishnu.")
            return englishMeanList
        }
    }
}